...
Installing directly from a local path, a `file://` URL, or a Git URL (GitHub, GitLab, Bitbucket, or any generic host) works for any package that contains `pnlx.json`, as does installing by bare name against a repository's `repository-index.json`. Declared package `dependencies` are resolved at install time — a version a dependency pins that conflicts with an already-installed version is reported rather than silently mis-resolved — and repository indexes can be signed and verified (`pnl repo sign` plus a `--key` on the repository). A package's native libraries and headers can also be fetched over `http(s)`, `ftp`, `ftps`, or `git` (see [Native library discovery](configuration.md#native-library-discovery)). Full backtracking version solving across repositories is still design-stage work. pnl never redistributes the C libraries or their headers — it generates bindings locally from what is on your machine — so a working `libclang` is required at install time (see [Installation](installation.md)).
