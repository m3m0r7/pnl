...
`pnlx.json` を含むパッケージであれば、ローカルのパス・`file://`・Git/GitHub の URL・配布アーカイブ（`.tar.gz`/`.zip`）から直接インストールできます。リポジトリの `repository-index.json` を使ったベース名指定のインストールも可能です。宣言された `dependencies` はインストール時に解決され、リポジトリインデックスは署名・検証できます（`pnl repo sign` とリポジトリ側の `--key`）。ネイティブライブラリとヘッダーは `http(s)`・`ftp`・`git` からも取得できます（[ネイティブライブラリの探索](configuration.md#ネイティブライブラリの探索)参照）。`ftps` ソースやリポジトリをまたいだ本格的なバージョン解決はまだ設計段階です。
