...
- **`pnlx`**: ライブラリのパッケージを「作る側」のツール。ラッパーの生成や、パッケージメタデータの検証を行います。
...
インストール時、`pnl` はネイティブライブラリと C ヘッダーを探し、PHP のラッパーと C FFI 定義を生成します。実行時には、`Pnlx` SDK が PHP の FFI 経由でネイティブライブラリを開き、あなたの呼び出しを C へ橋渡しします。
...
flowchart TD
    ...
    AL --> H["Pnlx\\Runtime"]
    H -->|PHP FFI がネイティブライブラリを開く| C
```

- **`pnl`** はパッケージの解決・インストールを担当し、**`pnlx`** はパッケージの作成とラッパー生成を担当します。
- 生成された PHP は `@pnlx/` 配下に置かれ、1 回の `require` で読み込めます。
...
`pnlx.json` を含むパッケージであれば、ローカルのパス・`file://`・Git/GitHub の URL・配布アーカイブ（`.tar.gz`/`.zip`）から直接インストールできます。リポジトリの `repository-index.json` を使ったベース名指定のインストールも可能です。宣言された `dependencies` はインストール時に解決されます（依存が固定したバージョンが既にインストール済みのバージョンと衝突する場合は、黙って誤解決せずエラーで報告します）。リポジトリインデックスは署名・検証できます（`pnl repo sign` とリポジトリ側の `--key`）。ネイティブライブラリとヘッダーは `http(s)`・`ftp`・`ftps`・`git` からも取得できます（[ネイティブライブラリの探索](configuration.md#ネイティブライブラリの探索)参照）。リポジトリをまたいだバックトラッキングによる本格的なバージョン解決はまだ設計段階です。pnl は C ライブラリやそのヘッダーを再配布せず、マシン上にあるものからローカルでバインディングを生成します。そのためインストール時に動作する `libclang` が必要です（[インストール](installation.md)参照）。
