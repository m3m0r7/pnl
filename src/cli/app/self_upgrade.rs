...
pub fn self_upgrade(_bin_dir: &Path, _home: Option<&Path>) -> Result<()> {
    bail!(
        ...
        crate::model::config::self_repository()
    )
}

#[cfg(unix)]
pub fn self_upgrade(bin_dir: &Path, home: Option<&Path>) -> Result<()> {
    crate::app::ui::heading("pnl", "self-upgrade");
    let started = std::time::Instant::now();

    let current = Version::parse(env!("CARGO_PKG_VERSION"))
        .context("failed to parse the running pnl version")?;
    let repository = crate::model::config::self_repository();

    crate::app::ui::step(&format!("fetching release tags from {repository}.git"));
    let Some(release) = crate::app::release::latest_live()? else {
    ...
    if crate::app::release::detect_install_kind() == crate::app::release::InstallKind::Standalone {
    ...
    crate::app::ui::step(&format!("downloading {tarball_url}"));
    let tarball = crate::sources::fetch::fetch_asset(&tarball_url)?;
    let source = extract_source_tarball(&tarball)?;

    crate::app::ui::step("building release binaries with cargo (this may take a while)");
    ...
    crate::app::ui::created("installed", &version_bin);
    drop(source);

    switch_current(&layout, &latest)?;
    crate::app::ui::created("activated", &layout.current_link());
...
fn link_binaries(bin_dir: &Path, layout: &Layout) -> Result<()> {
    ...
    for name in BINARIES {
        ...
        crate::app::ui::created("linked", &link);
    }
    Ok(())
}
