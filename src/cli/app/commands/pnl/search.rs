...
use crate::model::manifest::{PnlManifest, Repository, RepositoryIndex};
use crate::model::repository_index::{load_repository_index, local_repository_dir};
use crate::sources::git_source::{GitSource, install_git_source};
use crate::util::glob::package_name_matches;
use crate::util::io::read_or_default;
...
pub(super) fn search(root: &Path, pattern: Option<&str>) -> Result<()> {
    ...
    for repository in &repositories {
        match enumerate_repository(repository) {
            ...
            Err(error) => {
                crate::app::ui::warn(&format!("skipped repository {}: {error}", repository.url));
            }
        }
    }
    ...
    if found.is_empty() {
        match pattern {
            Some(pattern) => crate::app::ui::info(&format!("no packages match \"{pattern}\"")),
            None => crate::app::ui::info("no packages found in the configured repositories"),
        }
        return Ok(());
    }
    ...
    for (name, package) in &found {
        println!(
            ...
            crate::app::ui::dim(&package.repository),
        );
    }
    Ok(())
}
...
fn collect_packages(dir: &Path, depth: usize, out: &mut Vec<(String, Vec<String>)>) {
    ...
    for entry in entries.flatten() {
        ...
        let manifest = path.join(crate::model::config::PNLX_MANIFEST_FILE);
        if manifest.is_file() {
