...
use crate::model::manifest::{PnlLock, PnlxManifest, PnlxPathmap};
use crate::model::platform::current_platform;
use crate::model::validate::{ensure_platform_matches, validate_pnlx_pathmap_values};
use crate::util::io::{read_json, write_json};

pub(super) fn pnlx_workspace_dir(root: &Path) -> PathBuf {
    crate::model::workspace::workspace_dir(root)
}
...
pub(super) fn pnl_lock_path(root: &Path) -> PathBuf {
    ...
    root.join(crate::model::config::LOCK_FILE)
}

pub(super) fn pnlx_pathmap_path(root: &Path) -> PathBuf {
    pnlx_workspace_dir(root).join(crate::model::config::PATHMAP_FILE)
}
...
pub(super) fn write_pnlx_autoload(root: &Path) -> Result<()> {
    ...
    let manifest_path = fs::canonicalize(&manifest_path)
        ...
        .to_string_lossy()
        .into_owned();
    crate::codegen::generate_autoload_php(
        &workspace.join(crate::model::config::AUTOLOAD_FILE),
        env!("CARGO_PKG_VERSION"),
        &packages,
        &composites,
        &manifest_path,
    )?;
    crate::codegen::generate_ide_helper_php(&workspace.join("ide-helper.php"))?;
...
fn collect_composite_requires(root: &Path, workspace: &Path) -> Result<Vec<String>> {
    ...
    let manifest = read_json::<crate::model::manifest::PnlManifest>(&manifest_path)?;
...
fn write_sdk_runtime(root: &Path) -> Result<()> {
    write_embedded_dir(
        &crate::embed::sdk_assets::SDK_DIR,
...
fn lock_path_relative_to_workspace(root: &Path) -> String {
    let depth = Path::new(&crate::model::workspace::output_dir_name(root))
        .components()
        .count()
        .max(1);
...
pub(super) fn tree_sha256(root: &Path) -> Result<String> {
    ...
    for relative in &files {
        let file_hash = crate::util::hash::sha256_file(&root.join(relative))?;
...
fn should_skip_package_copy(source_root: &Path, source_path: &Path) -> bool {
    ...
    relative == Path::new(crate::model::config::GENERATED_DIR)
        || relative.starts_with(".git")
        || relative.starts_with(crate::model::workspace::output_dir_name(source_root).as_str())
}
...
mod tests {
    ...
    fn write_package(version_dir: &Path) {
        ...
        let manifest = crate::model::manifest::PnlxManifest {
            entrypoint: "index.php".to_owned(),
            ..crate::model::manifest::PnlxManifest::default()
        };
        crate::util::io::write_json(
            &version_dir.join(crate::model::config::PNLX_MANIFEST_FILE),
            &manifest,
        )
        .unwrap();
