...
use crate::model::manifest::{NativeRequirement, PnlManifest, PnlxManifest};
use crate::sources::git_source::{GitSource, install_git_source};
use crate::util::io::{read_json, read_or_default};
...
fn resolve_remote_manifest(root: &Path, target: &str) -> Result<(PnlxManifest, String)> {
    if is_bare_package_name(target) {
        ...
        for repository in resolved_repositories(&manifest) {
            ...
            crate::app::ui::debug(&format!("trying {candidate}"));
    ...
    crate::app::ui::debug(&format!("fetching manifest from {target}"));
...
fn print_info(manifest: &PnlxManifest, source: &str) {
    crate::app::ui::heading("pnl", &format!("info {}", manifest.name));
    ...
    field("Source", &crate::app::ui::dim(source).to_string());
...
fn print_install_steps() {
    ...
    crate::app::ui::step("install the native libraries/headers (commands below, if any)");
    crate::app::ui::step("resolve each native library and header from your library paths");
    crate::app::ui::step("generate PHP FFI bindings for the headers");
    crate::app::ui::step("record everything in the lockfile/pathmap");
}
...
fn print_native_dependencies(manifest: &PnlxManifest) {
    ...
    for (platform, entry) in &manifest.setup.install {
        println!("  {}", crate::app::ui::bold(platform));
        for command in &entry.commands {
            println!("    {} {command}", crate::app::ui::cyan("$"));
        }
        for check in &entry.check_if_exists {
            println!(
                "    {} {}",
                crate::app::ui::dim("already-present check:"),
                crate::app::ui::dim(check)
            );
        }
    }
}
...
fn print_requirements(manifest: &PnlxManifest) {
    ...
    for (key, requirement) in &manifest.native_libraries {
        println!(
            "  {} {}",
            crate::app::ui::bold(key),
            crate::app::ui::dim(&format!("(version {})", requirement.version))
        );
...
fn field(label: &str, value: &str) {
    println!(
        "  {} {value}",
        crate::app::ui::cyan(&format!("{label:<12}"))
    );
}

fn sub_field(label: &str, value: &str) {
    println!("    {} {value}", crate::app::ui::dim(&format!("{label}:")));
}

fn section(title: &str) {
    println!("\n  {}", crate::app::ui::bold(title));
}
