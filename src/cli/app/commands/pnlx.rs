...
use crate::model::validate::{validate_pnlx_manifest_values, validate_pnlx_workspace};
use crate::native::header_adapter::{HeaderAdapterOptions, cdef_from_header};
use crate::util::io::{read_json, write_json, write_json_if_missing};
...
pub fn run() -> Result<()> {
    let cli = Cli::parse();
    if cli.information {
        return crate::app::about::print_information(crate::app::about::Tool::Pnlx);
    }
    if cli.license {
...
fn gen_pnlx(root: &Path, options: GenOptions) -> Result<()> {
    ...
    let manifest: PnlxManifest = read_json(&root.join(crate::model::config::PNLX_MANIFEST_FILE))?;
    ...
    generate_all(&GenerateArtifacts {
        generated_dir: &root.join(crate::model::config::GENERATED_DIR),
        ...
        // resolution.
        definitions: &resolve_definition_defaults(&manifest.compile_options.definitions),
...
fn resolve_definition_defaults(
    definitions: &[crate::model::manifest::RequireDefinition],
) -> Vec<crate::model::manifest::ResolvedDefinition> {
    definitions
        .iter()
        .filter_map(|definition| {
            ...
            Some(crate::model::manifest::ResolvedDefinition {
...
pub(crate) fn generate_installed_package_artifacts(
    ...
    shim: Option<&crate::native::shim::ShimRequest>,
...
struct GenerateArtifacts<'a> {
    ...
    definitions: &'a [crate::model::manifest::ResolvedDefinition],
    ...
    shim: Option<&'a crate::native::shim::ShimRequest>,
}
...
fn generate_all(args: &GenerateArtifacts<'_>) -> Result<Option<std::path::PathBuf>> {
    ...
    let out = generated_dir.join(format!(
        "{}{}",
        args.artifact_stem,
        crate::model::config::FFI_FILE_SUFFIX
    ));
    let (
        cdef,
        constants,
    ...
    if let Some(request) = args.shim {
        let static_inlines: Vec<crate::native::header_adapter::UnsupportedFunction> =
        ...
        if !static_inlines.is_empty() {
            match crate::native::shim::build(request, &static_inlines) {
                crate::native::shim::ShimOutcome::Built(built) => {
                ...
                crate::native::shim::ShimOutcome::Empty => {}
                // The consumer enabled the option but has no compiler: fatal, with an
                // actionable message (it cannot be satisfied any other way).
                crate::native::shim::ShimOutcome::MissingCompiler { functions } => {
                    anyhow::bail!(crate::native::cc::compiler_not_found_message(
                        &request.package,
                        &functions
                    ));
                }
                ...
                // whole install.
                crate::native::shim::ShimOutcome::CompileFailed { functions, detail } => {
    ...
    let mut signatures = crate::codegen::parse_signatures_with_unsupported(
        &cdef,
        &unsupported_functions,
        &enum_names,
    );
    crate::codegen::apply_symbol_aliases(&mut signatures, &symbol_aliases);
    ...
    crate::codegen::apply_symbol_aliases(&mut signatures, &shim_aliases);
    ...
    generate_aliases_php(
        &generated_dir.join(crate::model::config::ALIASES_FILE),
        &signatures,
    )?;
    Ok(shim_library)
}
...
fn init_pnlx(root: &Path) -> Result<()> {
    let manifest_path = root.join(crate::model::config::PNLX_MANIFEST_FILE);
...
fn publish_pnlx(root: &Path) -> Result<()> {
    ...
    match crate::native::install_script::install_script_hash(root, &manifest)? {
        Some(hash) => {
            manifest.setup.build_script_hash = Some(hash.clone());
            write_json(&manifest_path, &manifest)?;
            crate::app::ui::success(&format!("stamped install_script_hash {hash}"));
        }
        None => {
            manifest.setup.build_script_hash = None;
            write_json(&manifest_path, &manifest)?;
            crate::app::ui::info("no install scripts declared; cleared install_script_hash");
        }
    }
