...
use crate::model::manifest::{PnlxHeader, PnlxManifest, PnlxPathmap, ResolvedHeader};
...
pub(super) fn resolve_headers_from_pathmap(
    ...
    let pathmap = read_json::<PnlxPathmap>(
        &crate::model::workspace::workspace_dir(&project_root)
            .join(crate::model::config::PATHMAP_FILE),
    )?;
    ...
    if let Some(requirement) = manifest.native_libraries.get(library_key)
...
pub(super) fn resolve_headers<'a>(
    ...
    if manifest.compile_options.headers.is_empty() {
        return Ok(Vec::new());
    }

    if manifest.compile_options.headers.len() == 1 {
        return Ok(vec![&manifest.compile_options.headers[0]]);
    }
...
pub(super) fn resolve_library_key(
    ...
    if manifest.native_libraries.len() == 1 {
        return Ok(manifest
            .native_libraries
            .keys()
            .next()
    ...
    for candidate in [target, package_leaf] {
        if manifest.native_libraries.contains_key(candidate) {
...
fn find_project_root(root: &Path) -> Option<PathBuf> {
    ...
    loop {
        if crate::model::workspace::workspace_dir(&current)
            .join(crate::model::config::PATHMAP_FILE)
            .is_file()
        {
            return Some(current);
