...
pub const CHECK_TTL: Duration = Duration::from_secs(crate::model::config::UPDATE_CHECK_TTL_SECONDS);

/// Cache key for the latest-release lookup under the shared cache.
const CACHE_KEY: &str = crate::model::config::UPDATE_CHECK_CACHE_KEY;

/// Set to any value to skip the startup update check entirely.
const OPT_OUT_ENV: &str = crate::model::config::UPDATE_CHECK_OPT_OUT_ENV;
...
fn latest_cached_or_live(ttl: Duration) -> Result<Option<Release>> {
    if let Some(cached) = crate::sources::cache::read_fresh::<CachedRelease>(CACHE_KEY, ttl)
...
pub fn latest_live() -> Result<Option<Release>> {
    let tags = list_remote_tags(&format!("{}.git", crate::model::config::self_repository()))?;
    let release = latest_release(&tags);
    if let Some(release) = &release {
        let _ = crate::sources::cache::write(
            CACHE_KEY,
            &CachedRelease {
...
fn list_remote_tags(url: &str) -> Result<Vec<String>> {
    ...
    callbacks.credentials(crate::sources::git_source::authenticate);
