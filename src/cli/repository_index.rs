...
use crate::manifest::{IndexPackageVersion, Repository, RepositoryIndex, RepositoryType};
...
pub fn select_package_version(
    ...
    for (version, entry) in &package.versions {
        let normalized = crate::validate::normalize_semver(version)?;
        if constraint
            .as_ref()
...
pub fn installed_version_satisfies(version: &str, constraint: &str) -> Result<bool> {
    let version = crate::validate::normalize_semver(version)?;
