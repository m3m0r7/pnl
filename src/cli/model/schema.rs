...
impl SchemaKind {
    pub fn from_path(path: &Path) -> Option<Self> {
        match path.file_name().and_then(|value| value.to_str()) {
            Some(crate::model::config::PNL_MANIFEST_FILE) => Some(Self::Pnl),
            Some(crate::model::config::PNLX_MANIFEST_FILE) => Some(Self::Pnlx),
            Some(crate::model::config::LOCK_FILE) => Some(Self::PnlxLock),
            Some(crate::model::config::PATHMAP_FILE) => Some(Self::PnlxPathmap),
