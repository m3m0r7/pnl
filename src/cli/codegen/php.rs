...
struct ArgView {
    ...
    /// A struct passed BY VALUE (`JSValue`, gdbm's `datum`) — not a pointer. Its
    /// wrapper holds a `T *`, so the marshaller dereferences to the struct value FFI
    /// passes by value (otherwise FFI rejects `struct T *` where `struct T` is wanted).
