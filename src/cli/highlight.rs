...
//! `pnl install` (see [`crate::ui::example_block`]). It is deliberately modest:
...
//! Styling is delegated to [`crate::ui`], so it is automatically disabled when
    ...
    while index < chars.len() {
        ...
        match ch {
            ...
            '/' if chars.get(index + 1) == Some(&'/') => {
                out.push_str(&crate::ui::dim(&chars[index..].iter().collect::<String>()));
                break;
            }
            '#' => {
                out.push_str(&crate::ui::dim(&chars[index..].iter().collect::<String>()));
            ...
            c if is_ident_start(c) => {
                ...
                if KEYWORDS.contains(&word.as_str()) {
                    out.push_str(&crate::ui::magenta(&word));
                } else if LITERALS.contains(&word.as_str()) {
                    out.push_str(&crate::ui::yellow(&word));
                } else {
                    out.push_str(&word);
                }
            }
