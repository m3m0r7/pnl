...
use crate::model::validate::{normalize_semver, validate_semver};
...
pub(crate) fn resolve_native_library(
    ...
    dirs.extend(manifest.library_paths.iter().map(PathBuf::from));
...
fn resolve_route_file(
    ...
    if is_tbd {
        ...
        let tbd = crate::native::tbd::parse_tbd(&content)
...
pub(crate) fn exported_symbols(library_path: &str) -> Option<BTreeSet<String>> {
    ...
    if library_path.ends_with(".tbd") {
        return crate::native::tbd::parse_tbd(&String::from_utf8_lossy(&data))
            .map(|tbd| tbd.symbols)
...
pub(crate) fn resolve_header_for_native(
    extension_root: &Path,
    installed_root: &Path,
    native_path: &str,
    key: &str,
...
pub(crate) fn generation_headers_from_resolved_header(
...
fn pkg_config_version(key: &str) -> Option<String> {
    crate::native::pkg_config::modversion(&pkg_config_modules(key)).map(|version| {
...
fn pkg_config_include_dirs(key: &str) -> Vec<PathBuf> {
    crate::native::pkg_config::include_dirs(&pkg_config_modules(key))
}
...
fn pkg_config_lib_dirs(key: &str) -> Vec<PathBuf> {
    crate::native::pkg_config::lib_dirs(&pkg_config_modules(key))
}
