...
use crate::model::validate::validate_sha256;
...
pub fn install_script_hash(package_root: &Path, manifest: &PnlxManifest) -> Result<Option<String>> {
    ...
    if let Some(script) = &manifest.setup.build_script {
...
pub fn verify_install_scripts(
    ...
    match manifest.setup.build_script_hash.as_deref() {
...
fn confirm_unverified(
    ...
    if allow_unverified {
        crate::app::ui::warn(message);
        crate::app::ui::warn("continuing because --allow-unverified-install-scripts was provided");
        return Ok(());
    }
...
pub fn resolve_package_relative_path(package_root: &Path, relative: &str) -> Result<PathBuf> {
    crate::model::validate::validate_relative_package_path("setup.build_script", relative)?;
    ...
    if !resolved.starts_with(&root) {
        bail!(
            "setup.build_script path {} resolves outside the package root",
            relative
        );
    }
    Ok(resolved)
...
mod tests {
    use super::*;
    use crate::model::manifest::{InstallationEntry, PnlxManifest};
    ...
    fn hashes_installation_commands_in_stable_order() {
        ...
        manifest.setup.install.insert(
            "linux".to_owned(),
            InstallationEntry {
                commands: vec!["apt-get install libfoo-dev".to_owned()],
    ...
    fn hashes_self_build_script_contents() {
        ...
        manifest.setup.build_script = None;
